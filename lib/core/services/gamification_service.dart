...
class GamificationService {
  ...
  Future<void> _updateDailyStreak() async {
    ...
    if (_state.lastActivityDate != null) {
      ...
      if (difference == 0) {
        ...
        // Seri kırıldı
        _state = _state.copyWith(dailyStreak: 1, lastActivityDate: now);
      }
    } else {
      // İlk aktivite
      _state = _state.copyWith(dailyStreak: 1, lastActivityDate: now);
  ...
  Future<List<Badge>> _checkReadingBadges(int totalArticlesRead) async {
    return _checkBadgesByCategory(BadgeCategory.reading, totalArticlesRead, [
      'first_read',
      'reader_10',
      'reader_50',
      'reader_100',
  ...
  Future<List<Badge>> _checkStreakBadges(int streak) async {
    return _checkBadgesByCategory(BadgeCategory.streak, streak, [
      'streak_3',
      'streak_7',
      'streak_30',
      'streak_100',
  ...
  Future<List<Badge>> _checkFavoriteBadges(int totalFavorites) async {
    return _checkBadgesByCategory(BadgeCategory.favorites, totalFavorites, [
      'favorite_1',
      'favorite_10',
      'favorite_50',
      'favorite_100',
  ...
  Future<List<Badge>> _checkShareBadges(int totalShares) async {
    return _checkBadgesByCategory(BadgeCategory.sharing, totalShares, [
      'share_1',
      'share_10',
      'share_50',
    ]);
  ...
  Future<List<Badge>> _checkSearchBadges(int totalSearches) async {
    return _checkBadgesByCategory(BadgeCategory.exploration, totalSearches, [
      'search_10',
    ]);
  }
  ...
  Future<List<Badge>> _checkLevelBadges(int level) async {
    return _checkBadgesByCategory(BadgeCategory.achievement, level, [
      'level_5',
      'level_10',
      'level_25',
      'level_50',
  ...
  Future<List<Badge>> _checkPointsBadges(int totalPoints) async {
    return _checkBadgesByCategory(BadgeCategory.achievement, totalPoints, [
      'points_1000',
      'points_10000',
    ]);
  }
  ...
  List<Badge> getRecentlyUnlockedBadges({int limit = 5}) {
    ...
    unlocked.sort(
      (a, b) => (b.unlockedAt ?? DateTime(2000)).compareTo(

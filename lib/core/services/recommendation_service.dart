    ...
    try {
      ...
      for (final analytics in monthlyAnalytics) {
        ...
        for (final entry in analytics.sourcesRead.entries) {
          sourceCounts[entry.key] =
              (sourceCounts[entry.key] ?? 0) + entry.value;
        }
      }
      ...
      final favoriteCategories = topCategories
          .take(3)
          .map((e) => e.key)
          .toList();
...
class _RecommendedArticle {
  ...
  _RecommendedArticle({required this.article, required this.score});
}

...
class ReadingModeSettings {
  ...
  factory ReadingModeSettings.fromMap(Map<String, dynamic> map) {
    return ReadingModeSettings(
      ...
      backgroundColor:
          ReadingBackgroundColor.values[map['backgroundColor'] ?? 0],
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      if (savedMap != null && savedMap is Map) {
        state = ReadingModeSettings.fromMap(
          Map<String, dynamic>.from(savedMap),
        );
        debugPrint(
          '📖 Reading mode settings loaded: ${state.backgroundColorName}, ${state.fontSize}x',
        );
      }
    } catch (e) {
...
final readingModeProvider =
    StateNotifierProvider<ReadingModeNotifier, ReadingModeSettings>((ref) {

...
class CategoryOrderState {
  ...
  const CategoryOrderState({required this.categoryIds, this.isLoading = false});

  CategoryOrderState copyWith({List<String>? categoryIds, bool? isLoading}) {
  ...
  static CategoryOrderState get initial {
    ...
    final digerKategoriler = allCategories
        .where((c) => c.id != 'genel')
        .toList();
  ...
  Future<void> _loadCategoryOrder() async {
    try {
      ...
      if (savedOrder != null && savedOrder.isNotEmpty) {
        ...
        final savedOrderList = (savedOrder as List)
            .map((e) => e.toString())
            .toList();
        final validIds = savedOrderList
        ...
        final missingIds = defaultIds
            .where((id) => id != 'genel' && !orderedIds.contains(id))
            .toList();
...
final categoryOrderProvider =
    StateNotifierProvider<CategoryOrderNotifier, CategoryOrderState>((ref) {
